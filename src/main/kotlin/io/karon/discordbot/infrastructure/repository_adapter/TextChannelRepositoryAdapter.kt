...
class TextChannelRepositoryAdapter(private val jda: JDA) : TextChannelRepository {
	override fun getTextChannelByName(channelName: String): GuildTextChannel? {
		return jda.getTextChannelsByName(channelName, false).firstOrNull()?.let {
			GuildTextChannelAdapter(it)
		}
	}

	override fun createPrivateTextChannel(channelName: String, categoryName: String, onSuccess: (GuildTextChannel) -> Unit) {
